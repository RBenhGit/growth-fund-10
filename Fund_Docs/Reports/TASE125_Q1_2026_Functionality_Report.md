...
> ⚠️ **מסמך היסטורי / לא מעודכן — אינו משקף את התנהגות המערכת הנוכחית.**
> תמונת מצב מ-**2026-02-01** המתארת גרסה מוקדמת יותר של מנגנון הניקוד. מאז השתנו, בין היתר:
> - **ניקוד פוטנציאל** היה `צמיחה 50% / מומנטום 30% / הערכת שווי 20%` → כעת **`צמיחה עתידית 80% / מומנטום 20%`**, רכיב ה-PE הוסר לחלוטין.
> - **נורמליזציה** הייתה **min-max** → כעת **rank-percentile** (0–100).
> - **מומנטום** היה "תשואה מאז המחיר הישן ביותר" → כעת **חלון ~12 חודשים עקבי** (המחיר הקרוב ביותר ל-תאריך אחרון − 365 יום).
> - כשירות בסיס דורשת כעת גם 5 שנות היסטוריית הכנסות והון עצמי לא-שלילי.
>
> למתודולוגיה **עדכנית** ראו [CLAUDE.md](../../CLAUDE.md), [README.md](../../README.md) ורשומת `2026-07-10` ב-[CHANGELOG.md](../CHANGELOG.md). נשמר כרשומה היסטורית בלבד.
